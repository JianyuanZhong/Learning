...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		for(int j = 0; j < outputLayer.length; j++){
			NeuronUnit node_j = outputLayer[j];
			double in_j = node_j.getInputSum();
			double y_j = example.outputs[j];
			double a_j = outputLayer[j].getOutput();
			double delta_j = node_j.activation_derivative(in_j) * (y_j - a_j);
			layers[getNumLayers()-1][j].delta = delta_j;
		}
		// for l = L-1 to 1 do
		int L = getNumLayers();
		for(int l = L - 2; l >= 1 ; l--) {
			...
			for(int i = 0; i < layers[l].length; i++) {
				...
				NeuronUnit node_i = (NeuronUnit) layers[l][i];
				double in_i = node_i.getInputSum();
				double delta_i = node_i.activation_derivative(in_i) * sumPropregatedError(node_i);
				layers[l][i].delta = delta_i;
			}
		}
		...
		for(int i = L-1; i >= 1; i--) {
			for (int j = 0; j < layers[i].length; j++) {
				...
				Unit node_j = layers[i][j];
				for (Connection connection : node_j.incomingConnections) {
					// w_ij <- w_ij + alpha * a_i * delta_j
					double a_i = connection.src.getOutput();
					connection.weight = connection.weight + alpha * a_i * node_j.delta;
				}
			}
	...
	public double sumPropregatedError(NeuronUnit node_i){
		double sum = 0;
		for(Connection connection : node_i.outgoingConnections){
			sum += connection.weight * connection.dst.delta;
		}
		return sum;
	}
